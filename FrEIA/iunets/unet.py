...
from FrEIA.iunets import StandardBlock
...
class IUnetCOnvBlock(SequenceINN):
    ...
    def __init__(self,
        ...
        subnet_constructor = lambda ch_in, ch_out: StandardBlock.constructor(dim, ch_in, ch_out, **kwargs)
        for n_rep in range(depth):
            self.append(Fm.PermuteRandom)
            # self.append(Fm.HouseholderPerm, n_reflections=2)
...
class iUNet(GraphINN):
    def __init__(self,
        ...
        self.min_input_size = resampling_stride ** (len(channels) - 1)
        ...
        for num, (chs, arch, n_conv) in enumerate(zip(self.channels, self.architecture, self.conv_depth)):
            ...
            current_node = Ff.Node(current_node.out0,
                                   IUnetCOnvBlock,
                                   {'depth': arch, 'n_conv': n_conv, **convolution_args},
